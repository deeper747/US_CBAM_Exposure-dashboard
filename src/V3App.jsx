...
import { DATA_CUTOFF_YM, DEFAULT_FORECAST_ETS, EUR_USD, FORECAST_FROM, YTD_YEAR } from "./config/publicationConfig.js";
...
export function v4SectorYearCost(sec, yr, forecastEts, liveEntries = null) {
  const rows = RELEVANT.filter(d => d.sector === sec);
...
export function v4YtdCostFactorsForRows(rows, liveEntries = null) {
...
export default function V4App(){
  ...
  const ytdCostFactors=useMemo(()=>{
    ...
    for(const sec of SECTORS_LIST){
      const rows=RELEVANT.filter(d=>d.sector===sec);
      f[sec]=v4YtdCostFactorsForRows(rows,mergedTrade);
    }
    return f;
  },[mergedTrade]);
  ...
  const liveChartOverrides=useMemo(()=>{
    ...
    for(const ym of confirmedYms){
      ...
      for(const d of RELEVANT){
        ...
        factor+=(liveT!=null?liveT:avgMonthTonnes(d.cn,mo))*netMv;
      }
  ...
  const liveDataCutIdx=useMemo(()=>{
    ...
    if(!yms.length){
      // Static fallback: find DATA_CUTOFF_YM in V4_CHART_DATA
      const idx=V4_CHART_DATA.findIndex(p=>p.ym===DATA_CUTOFF_YM);
      return idx>=0?idx:0;
    }
    const latest=yms.sort().at(-1);
    const idx=V4_CHART_DATA.findIndex(p=>p.ym===latest);
    return idx>=0?idx:0;
  },[liveChartOverrides]);
  ...
  const chartPoints=useMemo(()=>V4_CHART_DATA.map(m=>({...m,v:(liveChartOverrides[m.ym]??m.factor)*getQtrEts(m.ym,ets)*EUR_USD/1e6})),[ets,liveChartOverrides]);

  const tableRows=useMemo(()=>SECTORS_LIST.map(sec=>{
    const s=SECTOR_STATS[sec];
    const{cfQ1,cfApr}=ytdCostFactors[sec];
    return{sec,...s,wBmg:SECTOR_BENCHMARKS[sec],taxQ1:cfQ1*Q1_ETS*EUR_USD,taxToday:(cfQ1*Q1_ETS+cfApr*ets)*EUR_USD};
  }),[ets,ytdCostFactors]);
  ...
  const displayTableRows=useMemo(()=>{
    ...
    return tableRows.map(r=>{
      ...
      return{
        ...
        displayCbam:v4SectorYearCost(r.sec,activeTableYear,ets,mergedTrade),
      };
    });
  ...
  const sectorAnnCosts=useMemo(()=>{
    ...
    const items=SECTORS_LIST.map(sec=>{
      let cost;
      if(yr){
        cost=v4SectorYearCost(sec,yr,ets,mergedTrade);
      } else if(rangeEnd!=="today"){
        let sum=0;
        for(let y=rangeStart;y<=Number(rangeEnd);y++) sum+=v4SectorYearCost(sec,y,ets,mergedTrade);
        cost=sum;
      } else{
  ...
  const annualCosts=useMemo(()=>{
    ...
    for(let y=rangeStart;y<=rangeEnd;y++){
      const yearData=V4_CHART_DATA.filter(m=>m.ym.startsWith(String(y)));
