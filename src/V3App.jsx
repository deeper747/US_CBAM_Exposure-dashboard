...
function JanFebModal({sec,mergedTrade,confirmedMos,rangeLabel,onClose}){
  ...
  const monthRows=useMemo(()=>confirmedMos.map(mo=>{
    ...
    return{label:_MO_ABB[parseInt(mo)-1],avg,y25,y26,
...
// ── YEAR-TO-DATE TRADE VALUE COMPARISON TABLE ─────────────────────────────────
function JanFebTable({mergedTrade,isMobile}){
  const [open,setOpen]=useState(true);
  const [modalSec,setModalSec]=useState(null);
  const [hovRow,setHovRow]=useState(null);

  // Derive confirmed months from live data; fall back to Jan–Feb when none yet
  ...
  // Section title tracks confirmed months
  const rangeLabel=confirmedMos.length===1
    ?_MO_ABB[0]
    :`${_MO_ABB[0]}–${_MO_ABB[confirmedMos.length-1]}`;
  ...
  return(
    <>
      ...
      <div style={{fontFamily:SANS,minHeight:"100vh",color:N.teal900,width:"100%",margin:0,background:N.teal900}}>
        ...
        <JanFebTable mergedTrade={mergedTrade} isMobile={isMobile}/>
